...
func (fcc *FiatCurrencyCreate) SetID(i int) *FiatCurrencyCreate {
...
func (fcc *FiatCurrencyCreate) defaults() error {
	...
	if _, ok := fcc.mutation.EntID(); !ok {
		if fiatcurrency.DefaultEntID == nil {
			return fmt.Errorf("ent: uninitialized fiatcurrency.DefaultEntID (forgotten import ent/runtime?)")
		}
...
func (fcc *FiatCurrencyCreate) check() error {
	...
	if _, ok := fcc.mutation.EntID(); !ok {
		return &ValidationError{Name: "ent_id", err: errors.New(`ent: missing required field "FiatCurrency.ent_id"`)}
	}
	return nil
}
...
func (u *FiatCurrencyUpsertOne) ID(ctx context.Context) (id int, err error) {
...
func (u *FiatCurrencyUpsertOne) IDX(ctx context.Context) int {
	id, err := u.ID(ctx)
	if err != nil {
		panic(err)
	}
